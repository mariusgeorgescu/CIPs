  ...
  **The spend redeemer SHOULD be unit** (`Constr 0 []`, same encoding as above). The validator does not inspect the redeemer; using a canonical redeemer improves recognisability and avoids unnecessary implementation divergence, but a non-unit redeemer does not by itself cause a TOA spend to fail.
  ...
  ### Mandatory Committed Artifacts

  Two hashes are distinguished throughout this CIP:

  - **Template hash** — the hash of the un-parameterised `TOA v1` validator (a single value committed in this CIP), concretely `blake2b_224(0x03 || unapplied_script_bytes)`. It identifies the TOA validator code itself, independently of any NFT.
  ...
  The compiled, **unapplied** Plutus V3 validator artifact, its build toolchain version(s), and its resulting template hash MUST be committed in the CIP repository alongside the Plinth source. The Plinth source alone is not sufficient to define the standard, because different compiler/toolchain versions may produce different UPLC bytes for the same source, which would change the template hash and therefore every derived address.

  | Artifact | Status |
  |---|---|
  | Plinth source (`validators/ToaV1.hs`) | **TODO** — required before transition to Active |
  | Compiled unapplied UPLC artifact (`validators/ToaV1.uplc` or equivalent) | **TODO** — required before transition to Active |
  | Pinned toolchain version(s) (GHC, plutus-tx / Plinth compiler, cardano-node ledger version) | **TODO** — required before transition to Active |
  | Template hash — `blake2b_224(0x03 \|\| unapplied_script_bytes)`, where `0x03` is the Plutus V3 script-hash language tag and `unapplied_script_bytes` is the ledger-serialised Plutus script form *before* parameter application (28-byte output). The same TextEnvelope / `cborHex` footgun described in *Address Derivation* applies here. | **TODO** — concrete 28-byte hex required before transition to Active |

  The compiled validator artifact is authoritative. Source-level implementation choices (which Plutus ledger API surface is used, which optimisation techniques are applied, etc.) are non-normative except insofar as they produce the committed compiled artifact and template hash. Such notes belong in the validator's own README, audit notes, or an implementation appendix.
  ...
  **MAY:**
  ...
  - the compiled, **unapplied** Plutus V3 validator artifact and its documented template hash (see *Mandatory Committed Artifacts*);
  ...
  The CIP repository MUST contain the **minimal normative artifacts** required for interoperability:

  - the Plinth source `validators/ToaV1.hs` (required for auditability and reproducibility; conformance is determined by the compiled validator artifact and test vectors);
  - the compiled, unapplied Plutus V3 validator artifact (binary UPLC bytes);
  - its template hash and pinned toolchain version(s);
  - the parameter-encoding CDDL (in *Canonical Parameter Encoding*);
  - normative test vectors (see *Test Vector Format*).

  **Non-normative transaction-shape files, SDKs, generated bindings, demo frontends, and full reference implementations SHOULD live in external repositories linked from the CIP**, not inside the CIP folder. This includes any `toa.tx3` reference file, the Haskell + Atlas reference library, the demo frontend, and any supplementary language bindings.

  If CIP editors prefer external publication for any or all of the minimal normative artifacts, the affected artifacts MUST be published as an **immutable external release** (e.g. a tagged GitHub release, an IPFS pin, or a content-addressed archive) and **referenced by content hash** from the CIP. In that case, the CIP MUST identify each externally-published artifact by its **blake2b-256 (or stronger) content hash**, so any implementer can verify they hold the exact bytes the standard refers to. **The validator artifact specifically** (compiled, unapplied UPLC) MUST be identifiable by an immutable content hash regardless of where it is stored, because every derived TOA address depends on it byte-for-byte.

  #### Reference validator (Plinth)

  The canonical validator is written in **Plinth** (Haskell compiled to Plutus Core) and lives at `validators/ToaV1.hs` in the CIP folder. Its compiled, un-applied script is committed alongside the source (see *Mandatory Committed Artifacts*), with the resulting template hash and pinned toolchain version(s) documented. Anyone can apply the canonical `TOAParamsV1` PlutusData value for a given `(toa_version, policy_id, asset_name)` and verify the resulting per-NFT script hash matches the address-derivation algorithm.
  ...
  A non-normative Haskell reference off-chain library is published externally and linked from this CIP. Repository: **TODO** — URL required before transition to Active.
  ...
  A non-normative reference frontend exercising address derivation, deposit, spend, and inspection is hosted externally and linked from this CIP. Demo app: **TODO** — URL required before transition to Active.

  ### Test Vector Format

  A normative test-vector file (`test-vectors/toa-v1.json`) ships in the CIP folder.

  Test vectors are split into two categories with **distinct conformance rules**:

  - **(a) Address-derivation vectors.** Implementations MUST reproduce the `expected_script_hash`, `expected_address_mainnet`, and `expected_address_testnet` fields **byte-for-byte** for the corresponding `(toa_version, policy_id, asset_name)`.
  - **(b) Validator scenarios.** Each scenario specifies the situation (inputs, outputs, mint, datums, redeemers), the expected pass/fail result, and an expected upper bound on transaction-size and execution-unit usage under the pinned protocol parameters (see below). Implementations need not produce byte-identical transaction bodies unless a scenario explicitly includes canonical transaction CBOR — different transaction builders may legitimately differ on input ordering, fee balancing, change outputs, collateral selection, and reference-script handling.

  **Pinned protocol parameters.** The CIP pins a specific protocol-parameters snapshot (Conway era) against which validator-scenario budget bounds are evaluated. `maxTxExUnits` and `maxTxSize` are part of this snapshot. The concrete snapshot is a **TODO** — required before transition to Active. Without this pin, value-shape stress cases have no objective pass criterion.
  ...
  [
    {
      "name": "ascii-asset-name",
      "plutus_version": "V3",
      "toa_version": 1,
      "policy_id": "<28-byte hex>",
      "asset_name_hex": "48656c6c6f",
      "cip14_fingerprint": "asset1...",
      "stake_credential": null,

      "params_cbor_hex": "<canonical PlutusData CBOR of TOAParamsV1 — TODO>",
      "unapplied_script_hash": "<28-byte hex; template hash before parameter application — TODO>",
      "applied_script_cbor_hex": "<CBOR of applied script bytes — the bytes that go into the tx `script` field — TODO>",

      "expected_script_hash": "<28-byte hex>",
      "expected_address_mainnet": "addr1...",
      "expected_address_testnet": "addr_test1...",
      "datum_policy": "unit_datum",
  ...
  - The `params_cbor_hex`, `unapplied_script_hash`, and `applied_script_cbor_hex` fields are **debug-aid fields**: optional in general, but **required for the first vector in each coverage category** (so implementers can locate failures along the encoding chain). Concretely:
    ...
    - `unapplied_script_hash` is the template hash *before* parameter application — `blake2b_224(0x03 || unapplied_script_bytes)`. A mismatch here points to a Plinth or toolchain version drift.
  ...
  - The `datum_policy` field documents the datum classification of the UTxO that the vector spends, from the four-way set: `unit_datum`, `inline_datum`, `datum_hash`, `no_datum` (per *Definitions — TOA deposit*). The MUST-BE-SPENDABLE rule for `inline_datum`, `datum_hash`, and `no_datum` vectors is defined in the *Datum and Redeemer Schema* section.
  - The `self_deposit_semantics` field documents whether the vector exercises the self-deposit case (`permissionless_spend_if_nft_is_at_own_toa`) or the ordinary case (`controlled_by_external_nft_holder`).

  Coverage MUST include at minimum:

  - ASCII asset name;
  - UTF-8 / non-ASCII Unicode asset name;
  - empty asset name;
  - maximum-length (32-byte) asset name;
  - [CIP-0067](https://cips.cardano.org/cip/CIP-0067) / [CIP-0068](https://cips.cardano.org/cip/CIP-0068) label 100 asset name (reference NFT);
  - CIP-0067 / CIP-0068 label 222 asset name (user NFT);
  - **self-deposit spend case** — the controlling NFT is at its own TOA and is spent permissionlessly;
  - **reference-input attack case** — the controlling NFT appears only in a reference input; regular spent inputs contain zero units; expected result: **fail** (the reference input does not satisfy `sumSpentInputs(ac) == 1`);
  - **datum-classification coverage — all four must be spendable:** (i) **Unit Datum**, (ii) **Inline Datum** (non-unit), (iii) **Datum Hash**, (iv) **No Datum**;
  - **permissive-policy warning fixture** — a vector documenting (as metadata, not validator proof) that the controlling minting policy permits further mints, so wallets and indexers can verify their warning UX against a concrete example.
  ...
  **Disposition:** No — reference scripts do not change the derived TOA address. The address is determined by the hash of the **applied** validator script, not by which UTxO carries the reference script. Reference scripts are safe and encouraged to reduce transaction size.
  ...
  - [ ] Canonical Plinth source for the TOA v1 validator published in the CIP folder.
  - [ ] Compiled, **unapplied** Plutus V3 validator artifact (binary UPLC bytes) published.
  - [ ] Compiler/toolchain versions pinned (GHC, plutus-tx / Plinth compiler, cardano-node ledger version, and any other tools required to reproduce the compiled artifact byte-for-byte).
  - [ ] Template hash documented in the CIP, computed as `blake2b_224(0x03 || unapplied_script_bytes)` per *Mandatory Committed Artifacts* (28-byte output).
  - [ ] Address-derivation helper published (Haskell reference; supplementary TypeScript/Rust ports encouraged but non-normative).
  - [ ] Normative test vectors published per *Test Vector Format* and verified against **at least two independent implementations**. Vector coverage (address-derivation, validator-rule negative, authorisation-scope, value-shape, spend, wallet/indexer fixtures) is defined in *Test Vector Format*.
  - [ ] At least one non-normative reference off-chain implementation published, including address derivation, deposit, spend, and NFT carry-through.
  - [ ] A testnet demonstration or reference workflow exercises address derivation, deposit, spend, and NFT carry-through.
  ...
  1. **Publish the canonical Plinth validator source** (`validators/ToaV1.hs`) and the compiled, unapplied Plutus V3 artifact (binary UPLC bytes), with pinned toolchain versions and the documented template hash. Identify the validator artifact by its immutable content hash (committed inside the CIP folder or published as an external immutable release, per *Reference Artifacts — What lives where*).
  2. **Publish the parameter-encoding CDDL** and confirm it round-trips through at least two independent CBOR libraries.
  3. **Publish the address-derivation test vectors** (category (a) in *Test Vector Format*) covering ASCII / non-ASCII Unicode / empty / max-length / CIP-0067 label 100 / CIP-0067 label 222 asset names.
  4. **Publish the validator-scenario test vectors** (category (b)) covering all positive, negative, authorisation-scope, value-shape stress, spend, and reference-input cases listed in *Acceptance Criteria*, with budget bounds evaluated against the pinned protocol-parameters snapshot.
  5. **Publish the address-derivation helper** (Haskell reference; supplementary TypeScript/Rust ports as non-normative).
  6. **Publish at least one non-normative reference off-chain implementation**, including address derivation, deposit, spend, and NFT carry-through.
